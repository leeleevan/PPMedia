...
use ffinst\flow\cache\LAFacebookCacheManager;
use ffinst\flow\FlowFlow;
...
class FFDBManager extends LADBManager{
	...
	public final function social_auth(){
		if (isset($_REQUEST['type'])){
			if ($_REQUEST['type'] == 'facebook'){
				...
				$facebook_сache = $this->context['facebook_сache'];
	...
	protected function saveGeneralSettings($settings){
		parent::saveGeneralSettings($settings);
	...
	protected function customizeResponce(&$responce){
		...
		$facebookCache = $this->context['facebook_сache'];
		if ($facebook_changed) {
			$facebookCache->clean();
		}
