...
var FlowFlowApp = (function($){
  ...
  StreamModel = Backbone.Model.extend({
    defaults: function () {
      return {
        ...
        "big":                   "nope",
        "show-profile":          "yep",
        "show-follow-button":       "yep",
        "profile-background-color": "rgb(250, 250, 250)",
        "profile-button-base-color": "rgb(60, 167, 115)",
        "profile-button-label-color": "rgb(255, 255, 255)",
        "profile-text-color": "rgb(60, 60, 60)",
        "profile-sec-color": "rgb(158, 158, 158)"
      }
    },
    initialize: function() {},
    save: function(isNew){
  ...
  StreamView = Backbone.View.extend({
    ...
    events: {
      ...
      "change .design-step-2 input[id*=show-profile]" : "optionsProfileToggle",
      "change .design-step-2 input[id*=show-follow-button]" : "optionsProfileButtonToggle"
    },
...
debugger
    ...
    handleUserProfile: function() {
        // disable profile if there is more than 1 feeds connected OR 1 feed connected, that is NOT username type
        var feeds = this.model.get('feeds')

        if( feeds.length > 1 || (feeds.length === 1 && feeds[0]['timeline-type'] != 'user_timeline')){
            this.$('.design-step-2 input[id*=show-profile]')
            .attr('disabled', 'disabled')
            .attr('checked', false)
            this.$('.show-profile-option').hide()
        } else {
            this.$('.design-step-2 input[id*=show-profile]')
            .attr('disabled', false)
            .attr('checked', this.model.get('show-profile') === 'yep' ? 'checked' : false)
            this.$('.show-profile-option')[this.model.get('show-profile') === 'yep' ? 'show' : 'hide']()
        }
    },
    ...
    optionsProfileToggle: function(){
        if(this.model.get('show-profile') == 'yep'){
            this.$('.show-profile-option').show()
        }else{
            this.$('.show-profile-option').hide()
        }

        Controller.setHeight(this.model.get('id'))
    },

    optionsProfileButtonToggle: function(){
        this.$('.profile-button-option').toggle()
    },
    ...
    render: function(){
      ...
      renderFilters: function(uid){
          ...
          includeArr.forEach(function (item, i) {
              var $label =
                  '<li class="filter-label">' + item +
                  '<i data-action="delete-filter" data-type="include" data-content="' + item + '" class="flaticon-feed_type_user"></i>' +
                  '</li>'
